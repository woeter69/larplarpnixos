...
# Notify user about the script launch (if notify-send is installed)
command -v notify-send &>/dev/null && notify-send -t 3000 -i info "   Checking for Wi-Fi..." || true
...
# Check Wi-Fi status
wifi_status=$(nmcli -t -f WIFI general 2>/dev/null | tail -n1 || echo "enabled")
...
case "$choice" in
    ...
    "Connected to"*)
        wezterm start -- sh -c "nmcli dev wifi show-password; echo 'Press Enter to close...'; read"
        ;;
